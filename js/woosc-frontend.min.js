...
(function (a) {
  ...
  a(function () {
    x(),
      ...
      q(),
      r(),
      woosc_vars.open_bar === "yes" && (b("bar"), h()),
    ...
    a(document).on("click touch", ".woosc-table-settings", function (b) {
      b.preventDefault(), a(".woosc-settings").toggleClass("open");
    }),
    a(document).on("click touch", ".woosc-bar-print", function (b) {
      b.preventDefault(), a.print("#woosc-area");
    }),
    a(document).on("keydown", function (b) {
      (b.ctrlKey == !0 || b.metaKey == !0) &&
        b.which == "80" &&
        a(".woosc-area").hasClass("woosc-area-open-table") &&
        (b.preventDefault(), a.print("#woosc-area"));
    }),
    ...
    a(document).on(
      ...
      function (d) {
        ...
        navigator.clipboard.writeText(c).then(
          function () {
            alert(woosc_vars.copied_text.replace("%s", c));
          },
          function () {
    ...
    a(document).on("click touch", ".woosc-bar-search", function (b) {
      if (
        ...
        return d != null && clearTimeout(d), (d = setTimeout(s, 300)), !1;
    }),
    a(document).on("click touch", ".woosc-popup", function (b) {
      a(b.target).closest(".woosc-popup-content").length === 0 &&
    ...
    a(document).on("click touch", ".woosc-popup-close", function (b) {
      b.preventDefault(), a(this).closest(".woosc-popup").toggleClass("open");
    }),
    a(document).on("woovr_selected", function (e, d) {
      var c, b;
      woosc_vars.variations === "yes" &&
        ((c = d.attr("data-id")),
        (b = d.attr("data-pid")),
        c > 0
          ? a(".woosc-btn-" + b)
    ...
    a(document).on("found_variation", function (c, d) {
      if (woosc_vars.variations === "yes") {
        ...
        a(".woosc-btn-" + b)
          .removeClass("woosc-btn-added woosc-added")
          .attr("data-id", d.variation_id),
          a(".woosc-btn-" + b + ":not(.woosc-btn-has-icon)").html(
            woosc_vars.button_text
          ),
          a(".woosc-btn-has-icon.woosc-btn-" + b)
            .find(".woosc-btn-icon")
    ...
    a(document).on("click touch", ".woosc-btn", function (l) {
      ...
      l.preventDefault(),
        ...
        c.addClass(""),
        c.hasClass("woosc-btn-added woosc-added")
          ? woosc_vars.click_again === "yes"
            ? (g(d),
              woosc_vars.button_action === "show_message" &&
                a.notiny({
                  theme: "woosc",
        ...
        woosc_vars.button_action === "show_bar" && (b("bar"), h()),
        woosc_vars.button_action === "show_table" && (b("table"), h(), i()),
        (woosc_vars.button_action === "show_message" ||
          woosc_vars.button_action === "none") &&
          a(".woosc-bar").hasClass("woosc-bar-open") &&
          b("bar"),
        woosc_vars.button_action === "show_sidebar" && (b("sidebar"), n());
    }),
    a(document).on(
      "click touch",
    ...
    a(document).on("click touch", ".woosc-bar-btn", function (c) {
      c.preventDefault(),
        a(".woosc-table-items").hasClass("woosc-table-items-loaded") ||
          b("table"),
        C();
    }),
    a(document).on("click touch", function (b) {
      (woosc_vars.click_outside === "yes" ||
        (woosc_vars.click_outside === "yes_empty" &&
          parseInt(a(".woosc-bar").attr("data-count")) === 0)) &&
        a(b.target).closest(".wpc_compare_count").length === 0 &&
        a(b.target).closest(".woosc-popup").length === 0 &&
        a(b.target).closest(".woosc-btn").length === 0 &&
        a(b.target).closest(".woosc-table").length === 0 &&
        a(b.target).closest(".woosc-bar").length === 0 &&
        a(b.target).closest(".woosc-menu-item a").length === 0 &&
        a(b.target).closest(".woosc-menu a").length === 0 &&
        a(b.target).closest(".woosc-sidebar-btn").length === 0 &&
        (woosc_vars.open_button === "" ||
          a(b.target).closest(woosc_vars.open_button).length === 0) &&
        o();
    }),
    ...
    woosc_vars.open_button !== "" &&
      a(document).on("click touch", woosc_vars.open_button, function (c) {
        c.preventDefault(),
          woosc_vars.open_button_action === "open_page"
            ? woosc_vars.page_url !== "" &&
              woosc_vars.page_url !== "#" &&
              (window.location.href = woosc_vars.page_url)
            : (woosc_vars.open_button_action === "open_popup" &&
                (a(".woosc-table-items").hasClass("woosc-table-items-loaded") ||
                  b("table"),
                h(),
                i()),
              woosc_vars.open_button_action === "open_sidebar" &&
                (a(".woosc-sidebar-items").hasClass(
                  "woosc-sidebar-items-loaded"
                ) || b("sidebar"),
                n()));
      }),
    a(document).on(
      ...
      function (c) {
        woosc_vars.menu_action === "open_popup" &&
          (c.preventDefault(),
          a(".woosc-table-items").hasClass("woosc-table-items-loaded") ||
            b("table"),
          h(),
          i()),
          woosc_vars.menu_action === "open_sidebar" &&
            (c.preventDefault(),
            a(".woosc-sidebar-items").hasClass("woosc-sidebar-items-loaded") ||
              b("sidebar"),
            n());
      }
    );
  ...
  function c(e) {
    for (
      var c = e + "=", d = document.cookie.split(";"), b = 0, a;
      b < d.length;
      b++
    ) {
      for (a = d[b]; a.charAt(0) == " "; ) a = a.substring(1);
      if (a.indexOf(c) == 0)
  ...
  function u() {
    ...
    woosc_vars.user_id !== "" &&
      ((f += "_" + woosc_vars.user_id), (g += "_" + woosc_vars.user_id)),
      a(".woosc-settings-field").each(function () {
        var b = a(this).val();
        a(this).prop("checked")
          ? (c.push(b), a(".woosc_table .tr-" + b).removeClass("tr-hide"))
          : a(".woosc_table .tr-" + b).addClass("tr-hide");
      }),
      ...
      ? ((d = c(g).split(",")),
        d.length < woosc_vars.limit
          ? ((d = a.grep(d, function (a) {
            ...
            (k = d.join()),
            e(g, k, 7))
          : ((i = !0), (h = h.replace("{limit}", woosc_vars.limit))),
        (f = d.length))
      : (e(g, b, 7), (f = 1)),
      l(f),
      a(document.body).trigger("woosc_added", [f]),
      i
        ? (a('.woosc-btn[data-id="' + b + '"]').removeClass(
            "woosc-btn-adding woosc-adding"
          ),
          alert(h))
        : (a('.woosc-btn[data-id="' + b + '"]')
            .removeClass("woosc-btn-adding woosc-adding")
            .addClass("woosc-btn-added woosc-added"),
          a('.woosc-btn[data-id="' + b + '"]:not(.woosc-btn-has-icon)').html(
            woosc_vars.button_text_added
          ),
          a('.woosc-btn-has-icon[data-id="' + b + '"]')
            .find(".woosc-btn-icon")
  ...
  function m() {
    ...
    c(b) != "" &&
      ...
      a(".woosc-btn").removeClass("woosc-btn-added woosc-added"),
      a(".woosc-btn:not(.woosc-btn-has-icon)").html(woosc_vars.button_text),
      a(".woosc-btn.woosc-btn-has-icon")
        .find(".woosc-btn-icon")
        .removeClass(woosc_vars.button_added_icon)
        .addClass(woosc_vars.button_normal_icon),
      a(".woosc-btn.woosc-btn-has-icon")
  ...
  function b(b) {
    var c = { action: "woosc_load_data", get_data: b, nonce: woosc_vars.nonce };
    b === "table" && a(".woosc-table-inner").addClass("woosc-loading"),
      ...
      a.post(woosc_vars.ajax_url, c, function (c) {
        (b === "bar" || b === "table") &&
          (a(".woosc-bar-items").html(c.bar).addClass("woosc-bar-items-loaded"),
          ...
          b === "table" &&
            ...
            a(window).width() >= 768
              ? woosc_vars.freeze_column === "yes" &&
                ...
                ? a("#woosc_table").tableHeadFixer({ head: !0, left: 1 })
                : woosc_vars.freeze_column === "yes"
                ? a("#woosc_table").tableHeadFixer({ head: !1, left: 1 })
                : woosc_vars.freeze_row === "yes" &&
                  a("#woosc_table").tableHeadFixer({ head: !0 })
              : woosc_vars.freeze_row === "yes" &&
                a("#woosc_table").tableHeadFixer({ head: !0 }),
            woosc_vars.scrollbar === "yes" &&
              a(".woosc-table-items").perfectScrollbar({ theme: "wpc" }),
  ...
  function t() {
    ...
    b >= 1024 &&
      a(".woosc-quick-table .woosc_table").tableHeadFixer({
        head: !1,
        left: 2,
      }),
      b >= 768 &&
  ...
  function i() {
    f(),
      ...
      woosc_vars.bar_bubble === "yes" &&
        a(".woosc-bar").removeClass("woosc-bar-bubble"),
  ...
  function k() {
    a("#woosc-area").removeClass("woosc-area-open woosc-area-open-table"),
      ...
      woosc_vars.bar_bubble === "yes" &&
        a(".woosc-bar").addClass("woosc-bar-bubble"),
      a(document.body).trigger("woosc_table_close");
  }
  function C() {
    a(".woosc-table").hasClass("woosc-table-open") ? k() : i();
  }
  function z() {
  ...
  function y() {
    a("#woosc-area").hasClass("woosc-area-open") ? o() : z(),
  ...
  function l(b) {
    var c, d;
    b === "first" &&
      ((c = A()), c != "" ? ((d = c.split(",")), (b = d.length)) : (b = 0)),
      a(".woosc-menu-item").each(function () {
        a(this).hasClass("menu-item-type-woosc")
          ? a(this).find(".woosc-menu-item-inner").attr("data-count", b)
          : a(this)
  ...
  function v() {
    woosc_vars.hide_empty_row === "yes" &&
      a(".woosc_table > tbody > tr").each(function () {
        var b = a(this),
          c = 0,
          d = !0;
  ...
  function r() {
    a("#woosc_highlight_differences").prop("checked")
      ? a(".woosc_table > tbody > tr").each(function () {
          var b = a(this),
            c = 0,
  ...
  function q() {
    a("#woosc_hide_similarities").prop("checked")
      ? a(".woosc_table > tbody > tr").each(function () {
          var b = a(this),
            c = 0,
            e = a(this).children("td").eq(1).html(),
